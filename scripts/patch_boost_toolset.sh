...
new = (f'      # {mark}: --toolset names a toolset but pins no compiler path, so\n'
       ...
       '        # MSVC stamps the toolset path into the objects it emits, so the\n'
       '        # archive itself says which headers were used. That is evidence;\n'
       '        # everything before this was inference from a link error.\n'
       '        import re as _re\n'
