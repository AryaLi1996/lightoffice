...
new = (f'      # {mark}: --toolset names a toolset but pins no compiler path, so\n'
       ...
       '        # Release objects turned out NOT to carry a toolset path (run\n'
       '        # 34975933781 reported none), so this is kept only for the case\n'
       '        # where one is present -- it is not the evidence it was meant to\n'
       '        # be, and it says so rather than staying silent.\n'
       '        import re as _re\n'
       '        _vers = sorted(set(_re.findall(rb"14\\.\\d\\d\\.\\d{5}", _blob)))\n'
       '        for _v in _vers:\n'
       '          print("[lightoffice]   toolset stamp in archive: %s (x%d)"\n'
       '                % (_v.decode(), _blob.count(_v)))\n'
       '        if not _vers:\n'
       '          print("[lightoffice]   no toolset stamp found in the archive")\n'
       '        if 0 == _ctl:\n'
       ...
       '        elif 0 != _hits:\n'
       '          # Collect, then print AND write to a file. Printing alone was\n'
       '          # not enough: GitHub echoes the job-wide env (an 8KB PATH plus\n'
       '          # INCLUDE/LIB/LIBPATH) ahead of every step, and the log API\n'
       '          # returns only a ~45KB tail, so the summary step\'s own preamble\n'
       '          # pushed this block off the front. build.yml cats the file from\n'
       '          # a final step so it lands last, where the tail can reach it.\n'
       '          _dl = []\n'
       '          _dl.append("--- boost configuration as the build actually saw it ---")\n'
       '          _dl.append("user-config path: " + (_uc or "<none>"))\n'
       '          if _uc:\n'
       '            _dl.append("user-config exists: %s" % os.path.isfile(_uc))\n'
       '            if os.path.isfile(_uc):\n'
       '              _txt = open(_uc).read().strip()\n'
       '              _dl.append("user-config: " + _txt)\n'
       '              import re as _re2\n'
       '              _m = _re2.search(r"<setup-amd64>\\\"([^\\\"]+)\\\"", _txt)\n'
       '              if _m:\n'
       '                _sp = _m.group(1)\n'
       '                _dl.append("setup script: " + _sp)\n'
       '                _dl.append("setup script exists: %s" % os.path.isfile(_sp))\n'
       '                if os.path.isfile(_sp):\n'
       '                  for _ln in open(_sp).read().splitlines():\n'
       '                    _dl.append("  | " + _ln)\n'
       '              else:\n'
       '                _dl.append("no <setup-amd64> in the jam")\n'
       '          _dl.append("INCLUDE this process sees:")\n'
       '          for _d in os.environ.get("INCLUDE", "").split(";"):\n'
       '            if _d:\n'
       '              _dl.append("  " + _d)\n'
       '          _dl.append("archive: %s bytes, __std_mismatch=%d, control=%d"\n'
       '                     % (len(_blob), _hits, _ctl))\n'
       '          _dl.append("-------------------------------------------------------")\n'
       '          for _ln in _dl:\n'
       '            print("[lightoffice] " + _ln)\n'
       '          _dp = os.environ.get("LIGHTOFFICE_BOOST_DIAG", "")\n'
       '          if _dp:\n'
       '            try:\n'
       '              with open(_dp, "w") as _f:\n'
       '                _f.write("\\n".join(_dl) + "\\n")\n'
       '              print("[lightoffice] diagnostics written to " + _dp)\n'
       '            except Exception as _e:\n'
       '              print("[lightoffice] could not write " + _dp + ": %s" % _e)\n'
       '          sys.exit("[lightoffice] boost was compiled against a newer STL than "\n'
       '                   "the one linking it: " + _lo_lib + " references "\n'
       '                   "__std_mismatch_*, which only the VS 2022 era STL "\n'
       '                   "provides, while this build links v142.")\n'
       '        else:\n'
